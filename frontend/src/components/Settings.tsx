...
export default function Settings() {
  ...
  const [activeSection, setActiveSection] = useState<'models' | 'points' | 'kb' | 'about'>('models');
    ...
    <div className="flex-1 overflow-y-auto" style={{ background: '#F8F8FA', padding: '24px 32px' }}>
      <div style={{ maxWidth: '720px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: '4px', marginBottom: '20px', background: '#EDEDED', borderRadius: '10px', padding: '4px', width: 'fit-content' }}>
          {[{ id: 'models' as const, label: '模型配置', icon: '🤖' }, { id: 'points' as const, label: '积分充值', icon: '💎' }, { id: 'kb' as const, label: '知识库', icon: '📚' }, { id: 'about' as const, label: '关于', icon: 'ℹ️' }].map(tab => (
...
function AboutSection() {
  ...
  useEffect(() => {
    // 从 localStorage 读取已缓存的诊断码
    const cached = localStorage.getItem('shaoziclaw_diagnostic_code');
    if (cached) {
      setDiagnosticCode(cached);
    } else {
      // 尝试实时获取
  ...
  const handleGenerateDiagReport = async () => {
    setDiagLoading(true);
    setShowDiagReport(false);
    try {
      const { invoke } = await import('@tauri-apps/api/core');
      const uid = localStorage.getItem('shaoziclaw_user_id') || 'default';
      const report: any = await invoke('generate_user_diagnostic_report', { userId: uid });
      setDiagReport(report);
      setShowDiagReport(true);
    } catch (e) {
      console.error('[诊断报告] 生成失败:', e);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ ...cardStyle, padding: '18px 22px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
          <span style={{ fontSize: '18px' }}>🔍</span>
          <div style={{ flex: 1 }}>
            <h3 style={{ fontSize: '14px', fontWeight: 600, color: '#111827', margin: 0 }}>用户诊断</h3>
            <p style={{ fontSize: '11.5px', color: '#9CA3AF', marginTop: '2px' }}>遇到问题时，提供诊断码给客服可快速定位</p>
          </div>
        </div>

        {/* 诊断码展示 */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', borderRadius: '8px',
          background: '#F8FAFC', border: '1px solid #E2E8F0', marginBottom: '12px' }}>
          <span style={{ fontSize: '12px', color: '#64748B' }}>我的诊断码：</span>
          <span style={{ fontSize: '16px', fontWeight: 700, color: '#1A7D4E', letterSpacing: '2px', fontFamily: 'monospace' }}>
            {diagnosticCode || '获取中...'}
          </span>
          {diagnosticCode && (
            <button
              onClick={() => { navigator.clipboard.writeText(diagnosticCode).catch(() => {}); }}
              style={{ marginLeft: 'auto', padding: '3px 10px', borderRadius: '5px', fontSize: '11px',
                background: '#E6F7EF', color: '#1A7D4E', border: '1px solid #B8E6CD', cursor: 'pointer' }}>
              复制
            </button>
          )}
        </div>
        ...
        {/* 生成诊断报告按钮 */}
        <button onClick={handleGenerateDiagReport} disabled={diagLoading}
          style={{ width: '100%', padding: '9px 0', borderRadius: '8px', fontSize: '13px', fontWeight: 500,
            background: diagLoading ? '#E5E7EB' : '#1E293B', color: '#fff', border: 'none',
            cursor: diagLoading ? 'wait' : 'pointer', transition: 'background 0.2s',
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
          {diagLoading ? '⏳ 生成中...' : '📊 生成诊断报告'}
        </button>

        {/* 诊断报告展示 */}
        {showDiagReport && diagReport && (
          <div style={{ marginTop: '12px', padding: '14px', borderRadius: '8px',
            background: diagReport.error ? '#FEF2F2' : '#F0FDF4',
            border: `1px solid ${diagReport.error ? '#FECACA' : '#BBF7D0'}`,
            fontSize: '12px', color: diagReport.error ? '#991B1B' : '#166534' }}>
            {diagReport.error ? (
              <div>⚠️ 报告生成失败：{diagReport.error}</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <div style={{ fontWeight: 600, marginBottom: '4px' }}>📊 诊断报告 · {diagReport.diagnostic_code}</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '6px', marginBottom: '6px' }}>
                  {[
                    { label: '总日志', val: diagReport.total_logs ?? 0, color: '#1E293B' },
                    { label: '错误', val: diagReport.error_count ?? 0, color: '#DC2626' },
                    { label: '警告', val: diagReport.warn_count ?? 0, color: '#D97706' },
                    { label: '信息', val: diagReport.info_count ?? 0, color: '#059669' },
                  ].map(item => (
                    <div key={item.label} style={{ textAlign: 'center', padding: '6px 4px', borderRadius: '6px', background: '#fff', border: '1px solid #E2E8F0' }}>
                      <div style={{ fontSize: '15px', fontWeight: 700, color: item.color }}>{item.val}</div>
                      <div style={{ fontSize: '10px', color: '#9CA3AF' }}>{item.label}</div>
                    </div>
                  ))}
                </div>
                {diagReport.time_range && (
                  <div style={{ fontSize: '11px', color: '#6B7280' }}>
                    时间范围：{diagReport.time_range.start?.slice(0, 16) || '-'} ~ {diagReport.time_range.end?.slice(0, 16) || '-'}
                  </div>
                )}
                {diagReport.top_errors && diagReport.top_errors.length > 0 && (
                  <div>
                    <div style={{ fontWeight: 600, marginBottom: '3px' }}>Top 错误：</div>
                    {diagReport.top_errors.slice(0, 3).map((e: any, i: number) => (
                      <div key={i} style={{ fontSize: '11px', color: '#374151', padding: '2px 0',
                ...
                <button onClick={() => setShowDiagReport(false)}
                  style={{ marginTop: '6px', padding: '4px 0', borderRadius: '5px', fontSize: '11px',

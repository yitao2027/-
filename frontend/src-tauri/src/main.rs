...
mod layered_memory; // 🧠 v5.5.34: 分层记忆模块（TencentDB-Agent-Memory理念落地，Task #22）
mod user_diagnostic; // 🔍 v5.5.34: 用户日志诊断模块（一人一码，Task #23）
...
/// v5.5.35: 添加直接HTTP回退检测，绕过Tauri updater插件可能的静默签名失败
#[tauri::command]
async fn check_update(app: tauri::AppHandle) -> Result<UpdateInfo, String> {
    ...
    match updater.check().await {
        Ok(Some(update)) => {
            ...
            log::info!("[Updater] Tauri插件检测到更新: v{}", update.version);
            Ok(UpdateInfo {
                available: true,
        ...
        Ok(None) => {
            // 🔧 v5.5.35：Tauri updater返回无更新，改用直接HTTP回退双重验证
            log::info!("[Updater] Tauri插件未检测到更新，启用HTTP回退检查...");
            match check_update_direct(&app).await {
                Ok(Some(info)) => {
                    log::warn!("[Updater] Tauri插件返回无更新但HTTP回退检测到新版本v{}！签名验证可能静默失败", 
                        info.version.as_deref().unwrap_or("?"));
                    Ok(info)
                }
                Ok(None) => {
                    log::info!("[Updater] HTTP回退也确认无更新，当前已是最新版本");
                    Ok(UpdateInfo { available: false, version: None, body: None, date: None, download_url: None })
                }
                Err(e) => {
                    log::error!("[Updater] HTTP回退检查也失败: {}", e);
                    Ok(UpdateInfo { available: false, version: None, body: None, date: None, download_url: None })
                }
            }
        }
        Err(e) => {
            ...
            log::error!("[Updater] check() failed: {}", err_str);
            // 🔧 v5.5.35: Tauri插件报错时也尝试HTTP回退
            log::info!("[Updater] Tauri插件报错，尝试HTTP回退...");
            match check_update_direct(&app).await {
                Ok(Some(info)) => {
                    log::warn!("[Updater] Tauri插件报错但HTTP回退检测到新版本v{}！", 
                        info.version.as_deref().unwrap_or("?"));
                    Ok(info)
                }
                _ => Err(format!("检查更新失败: {}", err_str)),
            }
        },
    }
}

/// 🔧 v5.5.35新增：直接HTTP获取latest.json，绕过Tauri updater插件的签名验证
/// 用于回退检测，确保不完全依赖Tauri插件的签名机制
async fn check_update_direct(app: &tauri::AppHandle) -> Result<Option<UpdateInfo>, String> {
    let current_version = app.package_info().version.to_string();
    let url = "https://releases.shaoziclaw.com/latest.json";

    log::info!("[Updater-Http] 当前版本: v{}, 获取: {}", current_version, url);
    ...
    if !resp.status().is_success() {
        return Err(format!("latest.json返回HTTP {}", resp.status()));
    }

    let json: serde_json::Value = resp.json().await
        .map_err(|e| format!("解析latest.json失败: {}", e))?;

    let latest_version = json.get("version")
        .and_then(|v| v.as_str())
        .unwrap_or("0.0.0");

    log::info!("[Updater-Http] 最新版本: v{}", latest_version);

    // 简单版本比较：按点分段逐位比较
    if !is_newer_version(latest_version, &current_version) {
        log::info!("[Updater-Http] 当前版本v{} >= 最新版本v{}，无需更新", current_version, latest_version);
        return Ok(None);
    }

    // 获取macOS平台的下载地址
    #[cfg(target_os = "macos")]
    let download_url = json.get("platforms")
        .and_then(|p| p.get("darwin-aarch64"))
    ...
    log::info!("[Updater-Http] 检测到新版本v{} (当前v{})，下载地址: {}", 
        latest_version, current_version, download_url.unwrap_or("无"));
...
fn main() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            // 🧠 v5.5.34: 分层记忆 Commands (Task #22)
            layered_memory::store_atomic_fact,
            layered_memory::store_atomic_facts,
            layered_memory::store_scenario_block,
            layered_memory::get_layered_context,
            layered_memory::refresh_user_persona,
            layered_memory::offload_long_content,
            layered_memory::generate_mermaid_states,
            layered_memory::get_atomic_facts,
            // 🔍 v5.5.34: 用户日志诊断 Commands (Task #23)
            user_diagnostic::get_diagnostic_code,
            user_diagnostic::find_user_by_code,
            user_diagnostic::get_user_diagnostic_logs,
            user_diagnostic::generate_user_diagnostic_report,
            user_diagnostic::generate_report_by_code,
        ])
.setup(|app| {

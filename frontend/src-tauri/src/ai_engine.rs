...
"#.to_string())
    ...
    let map_context: Option<String> = {
        ...
        if should_fetch_map {
            ...
            // v4.9.8: 整体超时45秒（单次请求10s+重试，留足余量）
            ...
            match map_result {
                ...
                Err(_) => {
                    // 超时：30秒内未完成，放弃地图数据继续流程
                    emit_log("⚠️ 地图API获取超时(30s)，跳过地图数据");
...
/// v5.5.9: 修复 UTF-8 不安全截断（byte index 在多字节字符中间会 panic）
fn truncate_for_log(s: &str, max_len: usize) -> String {
    if s.len() <= max_len { s.to_string() } else { format!("{}...", s.chars().take(max_len).collect::<String>()) }
}

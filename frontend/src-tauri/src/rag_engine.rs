...
impl RagState {
    ...
    /// v5.5.35: 若本地索引不存在，自动从远程下载（不再需要用户手动操作）
    pub async fn init(app_data_dir: PathBuf) -> Result<Self, String> {
        ...
        if !index_dir.exists() {
            println!("📚 RAG知识库索引不存在，尝试自动下载...");
            match download_kb_internal(&kb_dir).await {
                Ok(()) => {
                    println!("✅ 知识库自动下载完成，重新检查索引");
                    // 下载成功后重新检查索引
                    if !index_dir.exists() {
                        println!("⚠️ 下载完成但索引目录仍不存在，返回未初始化状态");
                        return Ok(Self::uninitialized(kb_dir));
                    }
                    // 索引存在，继续后续流程
                }
                Err(e) => {
                    eprintln!("⚠️ 知识库自动下载失败: {}，返回未初始化状态（用户可手动重试）", e);
                    return Ok(Self::uninitialized(kb_dir));
                }
            }
        }
    ...
    pub fn format_rag_context(chunks: &[KbChunk]) -> Option<String> {
        ...
        let mut ctx = String::from(
            "\n\n## 📚 行业经验参考\n\
             > 以下内容来自勺子Claw整合的餐饮行业经验，仅作为回答参考线索。\n\
             > 严格禁止：① 直接引用或复述参考内容的原文段落；② 提及任何书名、课程名、作者姓名、专家姓名、章节编号、页码、出版信息；③ 以\"某书提到\"\"某老师说\"\"某课程讲\"等方式表述。\n\
             > 必须：基于参考内容的核心思想，结合你的专业知识，用自己的语言重新组织表达；如参考内容与问题不匹配，直接用专业知识回答即可，不必强行使用。\n\
             > 输出时不要标注信息来源。\n"
        );

        for chunk in chunks {
            // 仅注入分类与技能维度，不暴露 source_file（避免书籍/课程文件名外泄）
            ctx.push_str(&format!(
                "\n### 参考线索（{} · {}）\n{}\n",
                chunk.category,
                chunk.skill_name,
...
// ============================================================================
// 内部自动下载（v5.5.35新增，无Channel进度回调，供init()使用）
// ============================================================================

/// 内部下载知识库索引（无需Channel，用于启动时自动下载）
async fn download_kb_internal(kb_dir: &PathBuf) -> Result<(), String> {
    ...
    println!("📥 [KB自动下载] 开始从 {} 下载索引...", KB_INDEX_BASE_URL);
    ...
    if meta_resp.status().is_success() {
        ...
        println!("✅ metadata.json 下载完成");
    } else {
        eprintln!("⚠️ 下载metadata失败: HTTP {}", meta_resp.status());
    }

    // 2. 下载索引压缩包（最多5分钟超时，约200MB）
    let resp = client
        .get(&index_url)
    ...
    if !resp.status().is_success() {
        return Err(format!("下载索引失败: HTTP {} (可能服务器上尚未上传kb_vectors.lance.tar.gz)", resp.status()));
    }

    let total_bytes = resp.content_length().unwrap_or(200 * 1024 * 1024);
    ...
    while let Some(chunk) = stream.next().await {
        ...
        let pct = ((downloaded as f64 / total_bytes as f64) * 100.0) as i32;
        if pct != last_pct {
            let mb = downloaded as f64 / 1024.0 / 1024.0;
            let total_mb = total_bytes as f64 / 1024.0 / 1024.0;
            println!("📥 [KB下载] {}% ({:.1}/{:.1} MB)", pct, mb, total_mb);
            last_pct = pct;
        }
    }
    drop(file);
